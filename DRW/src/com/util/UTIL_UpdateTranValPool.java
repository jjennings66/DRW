...
public class UTIL_UpdateTranValPool implements IScript
{
    public void execute(IContainerContext context) throws OException
    {
    	String className = this.getClass().getSimpleName();
        int ret, tran_num, side;
        Transaction tran = Util.NULL_TRAN;
        Table tDeals=Table.tableNew();
        String sSQL="select tran_num from ab_tran where tran_status=3";
        LIB.execISql(tDeals, sSQL, className);
        
        int iNumRows = LIB.safeGetNumRows(tDeals);
		for (int iRowCounter = 1; iRowCounter <=iNumRows; iRowCounter++){
			tran_num=tDeals.getInt(1, iRowCounter);
			//OConsole.oprint("Tran num: "+ tran_num);
	        try{
	        	tran=Transaction.retrieve(tran_num);
	        } catch (OException exception) {
	        	OConsole.oprint("Failed to retrieve transaction "+tran_num);
	        }
	        boolean isChanged=false;
	        int iNumParams=tran.getNumParams();
	        //OConsole.oprint("Num of params: "+iNumParams);
	        for(int iParamsCounter=1;iParamsCounter<iNumParams;iParamsCounter++){	        	
	        	try{
	        		if(tran.isPhysicalSide(iParamsCounter)==1){
		        		//OConsole.oprint("Side: "+ iParamsCounter);
			        	String projIndexBefore=tran.getField(TRANF_FIELD.TRANF_PROJ_INDEX.toInt(),iParamsCounter);
				        //OConsole.oprint("BEFORE: "+projIndexBefore);
				        ret=tran.updateLocationForValuePoolChange(iParamsCounter);
				        String projIndexAfter=tran.getField(TRANF_FIELD.TRANF_PROJ_INDEX.toInt(),iParamsCounter);
				        //OConsole.oprint("AFTER: "+projIndexAfter);
				        
				        if(Str.iEqual(projIndexBefore, projIndexAfter)==0){
				        	isChanged=true;
				        }
	        		}
	        	} catch (OException exception){
		        	OConsole.oprint("FAILED to update val pool for " + tran_num+": "+ exception.getLocalizedMessage());
		        }
	        }        
	        try{
	        	if(isChanged==true){
	        		...
	        		OConsole.oprint("Insert by status succeeded for "+ tran_num);
	        	}
	        } catch (OException exception){
	        	OConsole.oprint("Insert by status FAILED for " + tran_num+": "+ exception.getLocalizedMessage());
	        	tran.destroy();
	        }
	        //}
        tran.destroy();
    ...
    public static class LIB {
    	...
    	public static final String VERSION_NUMBER = "V1.002 (13Apr2023)";
