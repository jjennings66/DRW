...
/*
Script Name: DRW_DealImportCancelAndPurgeTestDeals
Description:  DRW_DealImportCancelAndPurgeTestDeals

Revision History
06-Apr-2023   Brian  New Script
07-Jun-2023   Brian  1) Got rid of purge... now just cancels
                     2) Now looks for 'Book' of something with 'Auto' in it (case insensitive), such as AutoBookedDealImporter

*/

@PluginCategory(SCRIPT_CATEGORY_ENUM.SCRIPT_CAT_GENERIC)
@PluginType(SCRIPT_TYPE_ENUM.MAIN_SCRIPT)
public class DRW_DealImportCancelAndPurgeTestDeals implements IScript {
	
	// For Testing set to true
	// boolean bPurgeEveryDealOver30000 = false;

	public void execute(IContainerContext context) throws OException {
		 LIB.log("Do not run this script", this.getClass().getSimpleName());
	}
 
	static public class FUNC {
 
		public static void doEverything(INS_TYPE_ENUM insType, int iRunNumber, String className) throws OException {
			doEverything(insType.toInt(), iRunNumber, className);
		}

		public static void doEverything(int iInsType, int iRunNumber, String className) throws OException {
			
			Table tDeals = Util.NULL_TABLE;
			Table tTranNums = Util.NULL_TABLE;

			try {
			
            
            // for safety
				LIB.log("*NOT* running this for safety", className);
                Util.exitFail();
            
				String sInsType = Ref.getName(SHM_USR_TABLES_ENUM.INSTRUMENTS_TABLE, iInsType);
				LIB.log("Running for Ins Type: " + sInsType, className);

				
				tDeals = Table.tableNew();
				LIB.safeAddCol(tDeals, "deal_num", COL_TYPE_ENUM.COL_INT);
				LIB.safeAddCol(tDeals, "book", COL_TYPE_ENUM.COL_STRING);
				LIB.safeAddCol(tDeals, "tran_num", COL_TYPE_ENUM.COL_INT);
				LIB.safeAddCol(tDeals, "version_number", COL_TYPE_ENUM.COL_INT);
				LIB.safeAddCol(tDeals, "toolset", COL_TYPE_ENUM.COL_INT);
				LIB.safeAddCol(tDeals, "base_ins_type", COL_TYPE_ENUM.COL_INT);
				LIB.safeAddCol(tDeals, "ins_type", COL_TYPE_ENUM.COL_INT);
				LIB.safeAddCol(tDeals, "tran_status", COL_TYPE_ENUM.COL_INT);
				tDeals.defaultFormat();
				
				// 3 = validated, 5 is cancelled
				String sSQL = "SELECT \n" + 
						"a.deal_tracking_num, a.book, a.tran_num, a.version_number, a.toolset, a.base_ins_type, a.ins_type, a.tran_status\n" + 
						"FROM\n" + 
						"ab_tran a\n" + 
						"WHERE lower(book) like lower('%auto%') AND tran_status in (3, 5, 10) AND a.trade_flag = 1  AND a.base_ins_type in (" + iInsType + ")";
				LIB.execISql(tDeals, sSQL, false, className);
				
				LIB.log("Row Count of deals: " + LIB.safeGetNumRows(tDeals), className);
				 
				
				
				tTranNums = Table.tableNew();
				LIB.safeAddCol(tTranNums, "tran_num", COL_TYPE_ENUM.COL_INT);
				tDeals.copyColDistinct("tran_num", tTranNums, "tran_num");
				
//				Table tTranInfo = Table.tableNew();
//				LIB.safeAddCol(tTranInfo, "tran_num", COL_TYPE_ENUM.COL_INT);
//				LIB.safeAddCol(tTranInfo, "source_system", COL_TYPE_ENUM.COL_STRING);
//				String sWhat = "tran_num, value";
//				String sWhere = "type_name = 'SourceSystem' AND value = 'Quorum'";
//				LIB.loadFromDbWithWhatWhere(tTranInfo, "ab_tran_info_view", tTranNums, sWhat, sWhere, false, className);
	  			
			//	LIB.safeAddCol(tTranInfo, "in_scope", COL_TYPE_ENUM.COL_INT);
		//		LIB.safeSetColValInt(tTranInfo, "in_scope", CONST_VALUES.VALUE_OF_TRUE);
	 
				LIB.safeAddCol(tDeals, "in_scope", COL_TYPE_ENUM.COL_INT);
				LIB.safeSetColFormatAsRef(tDeals, "in_scope", SHM_USR_TABLES_ENUM.NO_YES_TABLE);
				LIB.safeSetColValInt(tDeals, "in_scope", CONST_VALUES.VALUE_OF_TRUE);

				LIB.safeSetColFormatAsRef(tDeals, "in_scope", SHM_USR_TABLES_ENUM.NO_YES_TABLE);
				LIB.safeSetColFormatAsRef(tDeals, "base_ins_type", SHM_USR_TABLES_ENUM.INSTRUMENTS_TABLE);

			//	sWhat = "in_scope";
		//		sWhere = "tran_num EQ $tran_num";
			//	LIB.select(tDeals, tTranInfo, sWhat, sWhere, false, className);
				 
				tDeals.setColTitle("in_scope", "In Scope\nFor Delete?");
				LIB.viewTable(tDeals, "deal list for " + sInsType + ", Num Rows: " + LIB.safeGetNumRows(tDeals));
				...
				// delete not in scope deals from the table
//				if (bPurgeEveryDealOver30000 == false) {
//					
//					DEBUG_LOGFILE.logToFile("This many rows *BEFORE* removing deals *not* in scope: " + LIB.safeGetNumRows(tDeals), iRunNumber, className);
//					tDeals.deleteWhereValue("in_scope", CONST_VALUES.VALUE_OF_FALSE);
//					DEBUG_LOGFILE.logToFile("This many rows *AFTER* removing deals *not* in scope: " + LIB.safeGetNumRows(tDeals), iRunNumber, className);
//				}
				
				 
				int iNumRows = LIB.safeGetNumRows(tDeals);
				for (int iCounter = 1; iCounter <= iNumRows; iCounter++) {
	 			 
					// post every 10 rows
					if (iCounter % 10 == 0 || iCounter == iNumRows) {
						try {
							String sStatus = "Purging deal " + iCounter + " of " + iNumRows;;
					...
					int iInScope = LIB.safeGetInt(tDeals, "in_scope", iCounter);
					
//					if (bPurgeEveryDealOver30000 == true) {
//						int iDealNum2 = LIB.safeGetInt(tDeals, "deal_num", iCounter);
//						if (iDealNum2 > 30000) {
//							iInScope = 1;
//						}
//					}
					...
					if (iInScope == CONST_VALUES.VALUE_OF_TRUE) {
						int iDealNum = LIB.safeGetInt(tDeals, "deal_num", iCounter);
						int iTranStatus = LIB.safeGetInt(tDeals, "tran_status", iCounter);
						int iTranNum = LIB.safeGetInt(tDeals, "tran_num", iCounter);
						int iVersionNumber = LIB.safeGetInt(tDeals, "version_number", iCounter);
						
						
						// for safety, do *not* purge anything
						boolean bDoPurge = false;
						
						// don't do this any more
						boolean bDoPurgeAmendedVersions = false;
						if (bDoPurgeAmendedVersions == true) {
							try {

								if (iTranStatus == TRAN_STATUS_ENUM.TRAN_STATUS_AMENDED.toInt()) {
			 						DataMaint.tranPurge(ARCHIVE_DATA_TYPES.ADT_PURGE_AMENDED_TRAN, iTranNum);
								}
							} catch (Exception e) {
								LIB.log("tranPurge(ARCHIVE_DATA_TYPES.ADT_PURGE_AMENDED_TRAN", e, className);
							}
						}
						
						try {
							LIB.log("About to cancel deal: " + iDealNum, className);

							if (iTranStatus == TRAN_STATUS_ENUM.TRAN_STATUS_VALIDATED.toInt()) {
								Transaction.cancel(iTranNum, iVersionNumber);
							}
						} catch (Exception e) {
							LIB.log("Transaction.cancel", e, className);
						}
	 
						if (bDoPurge == true) {
							try {
		 						LIB.log("About to purge deal: " + iDealNum, className);

		 						DataMaint.tranPurge(ARCHIVE_DATA_TYPES.ADT_PURGE_CANCELLED_DEAL, iTranNum);
							} catch (Exception e) {
								LIB.log("tranPurge(ARCHIVE_DATA_TYPES.ADT_PURGE_CANCELLED_DEAL", e, className);
							}
						}
	 					
						if (bDoPurge == false) {
							LIB.log("*NOT* purging this deal for safety, Deal: " + iDealNum, className);
						}
			...
			tDeals = LIB.destroyTable(tDeals);
...
public static class LIB {
	...
	public static final String VERSION_NUMBER = "V1.003 (07Jun2023)";
